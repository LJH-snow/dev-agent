...
| --- | --- | --- | --- |
| Total provider stream accumulation in `packages/model/src/openai.ts`, `anthropic.ts`, `gemini.ts`, and `ollama.ts` | The current guard is per-line only. `content`, tool-call arguments, and some provider-specific arrays accumulate for the whole response. `maxTokens` is sent as a provider hint, not enforced as a byte limit by this package. | `packages/model/tests/streaming.test.ts` proves line limits and normal streaming, but no test proves a total response cap. | `NEEDS-EVIDENCE`: decide whether the hard cap belongs in the provider package, the agent output budget, or both. |
| Code-search traversal and write-back in `packages/tools/src/code-search.ts` and `apps/cli/src/index-command.ts` | Individual source and persisted-index reads are capped at `16 MiB`; directory traversal, total file count, and generated index size are not capped. This is intentional whole-project indexing behavior today. | `packages/tools/tests/code-search-persisted.test.ts`, `packages/tools/tests/code-search-cache.test.ts`, and `apps/cli/tests/index-command.test.ts` cover per-file/per-index limits, incremental refresh, and corruption recovery. | `NEEDS-EVIDENCE`: establish a project-size/time contract before adding a cap that could silently omit source files. |
| Rollback directory inspection in `packages/tools/src/filesystem.ts` (`preflightRollback`) | `readSnapshot` caps regular files, but `readdir(directory)` materializes every entry to prove a created directory is still safe to remove. The directory is project-controlled and the check must not miss unexpected entries. | `packages/tools/tests/filesystem-changeset.test.ts` covers non-empty-directory conflicts, but not a large directory. | `NEEDS-EVIDENCE`: replace with bounded early-exit enumeration only after a test locks the exact conflict semantics. |
| CLI approval input in `apps/cli/src/index.ts` (`readLineFromStdin`) | The function stops at the first newline, but a producer that never sends a newline can grow `buffer` until EOF. Interactive use normally supplies a short answer. | Approval tests cover allow/deny/EOF behavior, not an unterminated oversized line. | `NEEDS-EVIDENCE`: define a user-visible maximum and whether overflow should deny immediately or preserve the current EOF behavior. |
...
| `packages/model/src/*` total stream accumulation | `NEEDS-EVIDENCE` | The package has a per-line cap but no total-response contract. A follow-up must first choose the compatibility boundary. |
| `packages/tools/src/code-search.ts` traversal/write-back | `NEEDS-EVIDENCE` | Per-file and persisted-index reads are bounded; a total project cap would change whole-project indexing semantics and needs a product decision. |
| `packages/tools/src/filesystem.ts:preflightRollback` | `NEEDS-EVIDENCE` | Full directory inspection is required for the current no-surprise rollback invariant; a bounded replacement needs a dedicated proof. |
| `apps/cli/src/index.ts:readLineFromStdin` | `NEEDS-EVIDENCE` | Overflow semantics for an interactive approval answer are not defined, so a cap would change the approval UX. |
...
The four `NEEDS-EVIDENCE` rows remain open and are not silently changed by
these fixes.
