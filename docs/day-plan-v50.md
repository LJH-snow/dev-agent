...
- [x] **Step 1: inventory。** 记录当前 DOM 顺序、按钮 disabled 条件、session switch、
  preview loading/error/stale、chat streaming 和 validation/rerun 状态。
- [x] **Step 2: define contract。** 明确 label、role、`aria-expanded`、`aria-live`、
  `:focus-visible`、键盘 activation、操作中禁用和恢复语义；不增加 raw error 详情。
- [x] **Step 3: write matrix。** 覆盖 empty session、unknown session、concurrent request、
...
- [x] **Step 1: RED tests。** 在现有轻量 HTML contract 中锁定 focus-visible、稳定 labels、
  button activation 和 disabled 状态；不引入重量级测试框架除非证据需要。
- [x] **Step 2: minimal implementation。** 修复 focus ring、控件分组、键盘顺序、status/live
  announcements 和可恢复的 async state；保持默认视觉风格。
- [x] **Step 3: browser regression。** 用 in-app browser 验证 desktop 截图、Space/native
  activation、loading/error/success/stale、settled `aria-busy` 和 console health；窄窗口
  viewport matrix 因当前 harness 没有 viewport override 暂留后续。

## Task 2：异步 session 操作的可理解性

**Produces:** session 切换、rename/delete、streaming 与 preview 并发时的清晰状态。

- [x] **Step 1: model transitions。** 对每个操作定义 idle/loading/success/error/cancelled/stale
  状态，禁止旧响应覆盖新 session。
- [x] **Step 2: TDD only where needed。** 若发现真实状态丢失或误导，先写失败 contract，再做
  最小修复；若当前行为已满足则记录 Preserve，不做重构。
- [x] **Step 3: no new authority。** 不把 status/usage/bytes/counts 解释为执行、验证、恢复、
  回滚或 Undo 授权。
...
- [x] **Step 2: docs and decision。** 更新 README、CHANGELOG、progress 和 accessibility/interaction
  decision doc；明确 Preserve/NO-GO 的需求边界。
- [x] **Step 3: commit/push and next plan。** 只有验证通过才发布；若无新的真实需求，继续保持
  preview/integrity surface 冻结。

## Acceptance checklist

- [x] 键盘路径、focus-visible、稳定 aria labels/live regions 和 async disabled 语义有证据。
- [x] session switch/rename/delete/chat/preview 并发下没有 stale response 串入或误导状态。
