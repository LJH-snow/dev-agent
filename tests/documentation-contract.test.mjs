...
test("v0.1.7 documents the Desktop change set ID length limit", () => {
  assert.match(desktopReadme, /change set id/i);
  assert.match(desktopReadme, /96/i);
  assert.match(desktopReadme, /400/i);
  assert.match(desktopCandidate, /change set id/i);
  assert.match(desktopCandidate, /96/i);
});

test("v0.1.7 documents the Desktop evidence query filter limit", () => {
  assert.match(desktopReadme, /evidence query filter/i);
  assert.match(desktopReadme, /96/i);
  assert.match(desktopReadme, /400/i);
  assert.match(desktopCandidate, /evidence query filter/i);
  assert.match(desktopCandidate, /96/i);
});
...
test("agent memory documents the 16 MiB file write limit", () => {
  assert.match(cliReadme, /memory file[\s\S]*write limit/i);
...
test("cli workflow documents the 16 MiB input file read limit", () => {
  ...
  assert.match(desktopCandidate, /plan and apply input files?[\s\S]*16 MiB/i);
